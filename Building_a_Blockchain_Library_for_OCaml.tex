...
{ time    : Time.t;
  ...
  prev    : K.t option}
	...
	This mechanism uses the concept of \textit{Miners} which are nodes that add random nonce values to blocks before computing the hash of the resulting block.
	...
	It also assumes that Miners can always be rewarded for Mining and this may not always be true, so this scheme is not suitable for use with Logan.
...
module Make(Config: I_ParticipantConfig): I_Participant with type t = Config.t = struct
  ...
	...
	In order to encorporate the notion of a replica, the process of adding commits to the blockchain needs to be altered slightly.
...
let rec start_leader () = 
	...
	In this sense, not only does Logan satisfy the second of my success criteria, but also the fifth.
	...
	Importantly, the additional latencies are around 3s which is small and would be an acceptable waiting time for most applications.
	In this case, the system's latencies are still well within the success criteria of being of the order of magnitude of seconds.\\
