...
module Make(Config: I_Config)(LogCoder: I_LogStringCoder): I_Blockchain with type t = LogCoder.t = struct 
  ...
end
	\end{lstlisting}
	\end{minipage}
	The above code is the technical specification that I used to define my project. 
	\texttt{add\_transaction\_to\_blockchain} will add a user defined type to the blockchain and then return a polymorphic variant type containing information about whether the operation was successful. 
	\texttt{I\_LogStringCoder} is a module that can be used to allow arbitrary data types to be added to the blockchain, so long as a function is provided to encode them as a string.
	Finally, \texttt{Make} is a functor which will accept a module for coding the strings, and a module for any other configuration details and will return a Blockchain module.
