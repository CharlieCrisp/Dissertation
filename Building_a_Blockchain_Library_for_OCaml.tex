...
let rec start_leader () = 
	...
	Throughput, measured in transactions per second (tps), is also a useful measure because it shows how well a system can cope with large loads.  
	...
	Latency stays at 10-15ms up until the system reaches about 70tps.
	...
	This is a really interesting result because it shows that Logan's logic functions well at throughputs of between 50 and 60tps, whilst maintaining low transaction latency.
	...
	However, despite these machine limitations, these figures are much better than my success criteria which suggested the system should be able to function with throughputs greater than 10tps, and with latencies of less than 10s.\\
	\begin{figure}
		\centering
	...
	\subsection{Logan's Default Performance}
	...
	Figure \ref{figs:remlatencysize} compares the latency of making a transaction and the size of the mempool, for different throughputs.
	...
	\end{figure}
	The figure demonstrates a causality where the increases in time taken to pull mempool updates causes and increase the number of updates pulled in the next cycle. 
	This increase has the knock on effect of increasing the time spent adding these updates to the blockchain.
	The increase in the latency of pulling mempool updates prevents the system from being able to add transactions to the blockchain in constant time.\\

	\subsection{Logan's Performance Using Pure Git}
	...
	This result shows that in this scenario, Logan still satisfies the success criteria of being able to acheive throughputs of 10tps and latencies less of the order of magnitude of seconds.
	\begin{figure}
		\centering
	...
	\subsection{Logan's Maximum Throughput Capacity}
	In the previous sections, I evaluated how transaction latencies evolve over time, and compared the results for different applied throughputs.
	...
	\end{figure}
	Figure \ref{figs:appliedobservedsingle} shows how the throughput of transactions made to the blockchain varies with the throughput of transactions made to a Participants mempool. 
	This is compared to a linear plot of the ideal performance where a Leader never becomes overloaded.
	This figure clearly shows that the system starts diverging from ideal performance at an applied throughput of around 12tps.
	The observed throughput then starts to plateau, remaining at around 15tps for any applied throughputs higher than 20tps.
	This demonstrates that an ideal sustained operating throughput is around 12tps transactions per second which is slightly better than the success criteria of 10tps.
	Although the system will be able to handle bursts of higher throughputs without performance actually getting worse, it will not be able to cope with these throughputs for sustained periods of time.

	\subsection{Achieving Constant Time Latencies}
	...
	\begin{figure}
		...
		\label{figs:tworems}
	\end{figure}
	An important feature of this graph is the fact that the slope of the plots have not really changed from the case with only one remote Participant.
	For a throughput of around 10tps, transaction latencies will still rise by around 5s over 5000.
	The biggest difference is the fact that transaction latencies now start at around 12s, whereas they were around 6s before.
	...
	\end{figure}
	Importantly, the initial transaction latency has not increased from the case when there is just one remote Participant in the network.
	In contrast to pure Git, Irmin's Git provides the functionality for pulling remote updates in parallel and consequently the initial transaction latency has not doubled in this case.
	This indicates that the number of nodes in the network will not hinder transaction latency so long as the Leader node is computationally powerful enough to process updates from mempools in parallel.\\

	Overall, the results from using Irmin's Git implementation and pure Git both show different trade offs in terms of initial transaction latency and increases in transaction latencies over time. 
	However, the benefits shown by each implementation are not mutually exlusive.
	A better implementation of Git should be able to pull mempool updates in parallel whilst achieving the same linear increases in latencies that pure Git acheives. 
	If I assume, as before, a better streaming implementation for mempool synchronisation, then these results show that it is feasible for Logan to be able to achieve constant latencies in the size of the blockchain, as well as the number of nodes in the network.
