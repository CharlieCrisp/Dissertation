...
{ time    : Time.t;
  ...
  prev    : K.t option}
	...
	This mechanism uses the concept of \textit{miners} which are nodes that add many different random \textit{nonce} values to blocks before computing the hash of the resulting block.
	...
	Participants can be sure, with increasing probability in the size of the blockchain, that a block is agreed upon by consensus if it is in the longest chain. \\
...
let rec start_leader () = 
	...
	\begin{figure}
		...
		\caption{Observed throughput with increasing applied throughput for a single local participant}
		\label{fig:singlocal}
	\end{figure}
	...
	\begin{figure}
		...
		\caption{Observed throughput with increasing applied throughput for a single remote participant using pure Git}
	...
	\begin{figure}
		...
		\caption{Observed throughput with increasing applied throughput for two remote participants using pure Git}
