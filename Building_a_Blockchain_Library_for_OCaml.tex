...
module Make(Config: I_Config): I_Blockchain with type t = Config.t = struct 
  ...
	...
	In this chapter, I will first give a general overview of the system architecture.
	I will then detail how I built the blockchain data structure using the structures exposed by Irmin and Ezirmin, as well as the bugs that I encountered in these codebases.
	...
	The Architecture for the system I have built can be split up into two main categories: the system from the perspective of a `Leader' node and from a `Participant' or `Worker' node.
	In this model, the Leader node has total responsibility for the state of the blockchain and for committing new transactions.
	Participants exist to request transactions to be added to the blockchain and also to view the blockchain. 
	Participants view the blockchain by pulling the latest copy   over a two-way ssh connection which is maintained with the Leader.
	Participants request for transactions to be committed by writing to their local mempool. 
	The Leader periodically polls the Participant mempools to find new transactions to add to the blockchain.
	...
	In Section \ref{datastructure} I will detail how the blockchain data structure was built and why it satisfies the necessary conditions for being considered a blockchain.
	I will also show how I build the mempool data structures and how updates from different mempools are merged over the network.
	In Section \ref{consensus} I will move onto explaining the research I completed when designing the consensus mechanism for this project. 
	I will present a naive approach to consensus and explain why this is flawed and will actually `miss' transactions.
	...
	\section{The Blockchain Data Structure}\label{datastructure}
...
{ time    : Time.t;
  ...
  prev    : K.t option}
	...
	\section{Consensus Algorithms}\label{consensus}
