...
module Make(Config: I_ParticipantConfig): I_Participant with type t = Config.t = struct
  ...
	...
	For Logan, this means that the time taken for Leader to pull updates from mempools, will increase over time.
	Additionally, Figure \ref{figs:gitpullsize} shows that the latency of using Git increases linearly with the number of transactions pulled.
	This is an intuitive result that is unfortunately unavoidable due to such factors as network delays and data compression.
	...
	Putting this together, Figure \ref{figs:gitreposize} shows that for a mempool size of roughly 5000 transactions, the latency of pulling new updates will be around 10 to 12 seconds.
	This aligns exactly with the equivalent latency shown in Figure \ref{figs:leaderdelays}.
	The linear increase in this transaction latency is demonstrated for a constant number of updates being pulled so I can conclude that whilst the increasing number of transactions being pulled can have an effect on the latency of Leader polls, the root cause is the linear increase in pull latency as the size of the mempool increases.
	I have also shown that this linear latency increase is due to the implementation of Ezirmin, and that it is reasonable to assume that a better implementation, i.e. pure Git, could achieve constant time pulls, in the size of the mempool. 
	Therefore, whilst Logan shows a linear increase in transaction latency with the size of the blockchain, I conclude that a better synchronisation implementation would allow Logan to achieve the success critera set out in the Requirements Analysis. \\

	To complete my evaluation of Logan's performance with a single remote Participant, I examined how the latency varied with throughput.
	I took each measurement for latency by adding a constant number of transactions to the mempool of the Participant at varying throughputs.
	In order to measure the performance of the system outside of the Ezirmin's synchronisation overheads shown by Figure \ref{figs:gitreposize}, I took each measurement with a clean mempool.
	Figure \ref{figs:onerem} shows the results of this experiment.
	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{figs/remote_and_local_thrlat.png}
		\caption{Latency variations against throughput with one remote Participant and one local Participant}
		\label{figs:onerem}
	\end{figure}
	For the case where only the remote Participant is adding transactions, latency varies between 4 and 16 seconds, increasing linearly from throughputs from 0 to roughly 30 tranactions per second. 
	It appears that latency levels off at around 30 transactions per second, but this is due to the fact that the throughput has become so high that the time within which transactions are made only spans one or two leader polls.
	The result of this is that the indications of latency are not indicative of the latency of transactions when throughput is sustained at these values.
	Beyond this throughput, the remote Participant is not capable of committing transactions to its mempool at a faster rate, and so the system reaches a maximum throughput before becoming overloaded.\\

	Figure \ref{figs:onerem} also shows the case where a local Participant on the Leader's machine is also adding transactions at the same rate as the remote Participant.
	In this case, the maximum achievable throughput for any given latency doubles. 
	This indicates that Logan is able to comfortably cope with this nubmer of transactions, and that the only limitation on its performance is due to network overheads which can be attributed to the Ezirmin implementation described earlier.
	In this case, the system is also able to achieve more than twice the throughput of the single remote Participant case, as the Leader is run on a more powerful machine that is able to write transactions to disk faster.\\

	These results demonstrate that Logan is capable of sustaining throughputs of at least 100 transactions per second, in the case that a single remote Participant is used. 
	This means that the system much exceeds the success criteria of being able to sustain throughputs of a few transactions per second.
	...
	Figure \ref{figs:tworems} shows how the latency of making transactions varies with throughput.
	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{figs/two_remotes_and_part_thrlat.png}
		\caption{Latency variations against throughput with two remote Participants and one local Participant}
		\label{figs:tworems}
	\end{figure}
	This data was collected in the same way that the data was collected for Figure \ref{figs:onerem}, so the two graphs can be directly compared.
	For the case where there are just two remote Participants adding transactions, a throughput of 40 transactions per second will yield a latency of around 15 seconds. 
	This is equivalent to the latencies for the same throughput with just a single remote Participant, so adding an extra remote Participant has not increased the latency for a given throughput at all.
	However, where a single Participant was only able to achieve a maximum throughput of around 40 transactions per second, two Participants working in parallel can achieve more than twice that throughput.
	The reason that the leader can achieve almost no additional latency with an extra node is that Participant mempools all exist on isolated branches of the Leader's mempool.
	This means that it is possible to pull updates from all mempools in parallel. 
	Logan will wait until all of the updates have been pulled before collating the newly retrieved transactions.
	The result of this is that, so long as the leader is powerful enough to handle concurrent pulls, the entire update retrieval process will only take as long as the longest mempool retrieval time.
	The updates from the Participant added in this experiment can be pulled in a comparable time to the the updates from the first Participant, and therefore, there is very little percieved difference in latency.
	Figure \ref{figs:tworems} also shows that using a local Participant, throughputs can be increased majorly with no increase in latency. 
	
	\section{The Effects of Leader Failure}

	\chapter{Conclusion}
	For my Individual Part II project I have successfully built Logan, a blockchain library that allows developers to easily use distributed blockchain data structures in their applications.
	...
	\section{Irmin as a Mempool Data Structure}
	...
	\section{Failure Tolerance and Changing Leaders}
	...
	An alternative would be to use a HTTP protocol and I would like to investigate whether this would have an effect on the performance of the platform.
