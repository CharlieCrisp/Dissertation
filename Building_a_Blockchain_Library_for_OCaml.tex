...
module Log(AO : Irmin.AO_MAKER)(V: Tc.S0) = struct
  ...
	...
	\subsubsection*{Lwt}
	Lwt \cite{Lwt} is a library which I spent some time familiarising myself. 
...
module Make(Config: I_Config): I_Blockchain with type t = Config.t = struct 
  ...
	...
	Considering the criteria set out earlier for a data structure to be considered a blockchain, Irmin Stores satisfy DS\_1.
	Indeed, they also come close to satisfying DS\_2, as all blocks are indexed by the hash of their content. 
...
module type FS_Log = sig
  ...
  ...
	...
	Ezirmin Logs use Irmin blocks to store log entries, and therefore satisfy DS\_1. 
	To verify that they also satisfy DS\_2 I looked at the implementation of log items.
...
{ time    : Time.t;
  ...
  prev    : K.t option}
	...
	Therefore this satisfies DS\_2.
...
module Make(Config: I_ParticipantConfig): I_Participant with type t = Config.t = struct
  ...
	...
	They can be either \textit{active} if they fully duplicate the actions of another replica, or \textit{passive} if another machine completes some operation before transfering the result to that replica \cite{Replication}.
...
let rec start_leader () = 
	...
	\end{itemize}
	Of these success criteria, the \nameref{Implementation} chapter shows how SC\_1 is satisfied.
