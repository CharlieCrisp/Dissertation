...
{ time    : Time.t;
  ...
  prev    : K.t option}
	...
	Finally, I have introduced the notion of validation which allows the Leader nodes to accept or reject transactions depending on arbitrary conditions defined by the developer.
...
module Make(Config: I_ParticipantConfig): I_Participant with type t = Config.t = struct
  ...
	...
	\subsubsection*{Retrieving updates from mempools}
	...
	This will then motivate the second part of this section where I will give a more complex approach that ensures all transactions are committed in order.
	...
	The first (and most na{\"i}ve) approach I took to pulling updates from a number of participants, was sequentially pull updates from all the participants into separate branches of the leaders mempool.
	It is possible view newly retrieved updates for each mempool by maintaining a \textit{latest known} cursor that points to the latest transaction seen by the Leader in its last poll.
	All the updates from all of the mempools can be viewed in order as defined by the timestamps of log items that Ezirmin provides. 
	The diagrams in this section will treat the mempool as a merged log with transactions from all remote Participants because it makes the algorithm easier to explain and understand, without changing its semantics.
	...
	In this case, updates from the first worker will be pulled before the second, however, after the first pull has taken place, the first worker may have added additional transactions. 
	If these transactions are timestamped before the latest transaction in the second worker's mempool, then when the leader next polls for updates, transactions may be found which have a timestamp before the latest item committed to the blockchain.
	These items should have been committed first as they have an earlier time stamp.
	It is not clear whether or not these transactions should be committed so I shall refer to these transactions as \textit{missed} transactions, and all others at \textit{tracked} transactions.\\
	...
	\begin{enumerate}
		\item Missed transactions must have been added whilst the Leader is pulling updates. If they occurred before the pull had begun, then they would have been tracked. 
		Alternatively, missed transactions must occur after the latest known cursor from the previous pull. 
		\item Missed transactions must have been added at a point in time earlier than the latest transaction in the mempools.
